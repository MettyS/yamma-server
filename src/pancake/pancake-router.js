...
pancakeRouter
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    PancakeService.insertPancake(req.app.get('db'), newPancake)
      .then((pancake) => {
        res
...
pancakeRouter
  ...
  .all((req, res, next) => {
    ...
    //connect to the service to get the data
    PancakeService.getPancakeById(req.app.get('db'), req.params.pancake_id)
      .then((pancake) => {
        if (!pancake) {
          //if there is an error show it
          return res.status(404).json({
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    const numberOfValues = Object.values(pancakeToUpdate).filter(Boolean)
      .length;
    if (numberOfValues === 0) {
      //if there is an error show it
      return res.status(400).json({
  ...
  //relevant
  .delete((req, res, next) => {
    PancakeService.deletePancake(req.app.get('db'), req.params.pancake_id)
