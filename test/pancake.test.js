...
describe.skip('Pancake API:', function () {
  ...
  afterEach('cleanup', () =>
    db.raw('TRUNCATE TABLE pancake RESTART IDENTITY;')
  );
  ...
  describe('GET pancakes by id', () => {
    ...
    it('should return correct pancake when given an id', () => {
      ...
      return db('pancake')
        ...
        .then((_doc) => {
          doc = _doc;
          return supertest(app).get(`/api/pancakes/${doc.id}`).expect(200);
        })
        .then((res) => {
    ...
    it('should respond with a 404 when given an invalid id', () => {
      return supertest(app).get('/api/pancakes/aaaaaaaaaaaa').expect(404);
    });
  });
  ...
  describe('POST (create) new pancake', function () {
    ...
    it('should respond with 400 status when given bad data', function () {
      ...
      return supertest(app).post('/api/pancakes').send(badItem).expect(400);
    });
  });
  ...
  describe('DELETE a pancakes by id', () => {
    ...
    it('should delete an item by id', () => {
      return db('pancake')
        .first()
        .then((doc) => {
          return supertest(app).delete(`/api/pancakes/${doc.id}`).expect(204);
        });
    });
