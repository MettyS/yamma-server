...
jQuery(function ($) {
  ...
  const app = {
    ...
    render: function () {
      ...
      const pancakesFragments = pancakes.map((item) =>
        this.generatePancake(item)
      );
      ...
      $('#footer')
        .toggle(store.getAllPancakes().length > 0)
        .html(footerFragment);
    ...
    generateFooter: function () {
      ...
      return `
        <span id="pancake-count"><strong>${activePancakeCount}</strong> ${
        activePancakeCount === 1 ? 'item' : 'items'
      } left</span>
        <ul id="filters">
          <li>
            <a class="${
